...
def main(p,q,r):
    with Driver() as drv:
        ...
        th = 0
        h = (cal_p+16*p_div-1)//(16*p_div)
        w = (cal_r+64*r_div-1)//(64*r_div)
        for i in range(p_div):
            for j in range(r_div):
                uniforms[th, 1] = h if i != p_div-1 else (cal_p-i*h*16)//16
                uniforms[th, 2] = q
                uniforms[th, 3] = w if j != r_div-1 else (cal_r-j*w*64)//64
